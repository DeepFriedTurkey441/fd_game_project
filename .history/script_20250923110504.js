...
(function () {
  ...
  function addCluster(x) {
    ...
    const randScale = () => (0.7 + Math.random() * 0.9).toFixed(2); // wider height variance
    const mk = (href, xPos, yPos, speed) => {
      const el = createUse(href, { x: xPos, y: yPos, class: `sway ${speed||''}`.trim() });
      el.style.setProperty('--sx', '1');
      el.style.setProperty('--sy', randScale()); // scale Y only, origin at base
      return el;
    };
    g.appendChild(mk('#plantBlade', 0 + Number(jitter()), 396, ''));
    g.appendChild(mk('#leafBroad', 28 + Number(jitter()), 388, 'fast'));
    g.appendChild(mk(accent, 60 + Number(jitter()), 388, ''));
  ...
  function layoutPlants() {
    ...
    if (treasureGroup) {
      ...
      const x = Math.max(0, viewWidth - chestWidthVU - marginVU);
