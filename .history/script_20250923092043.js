...
(function () {
  ...
  function addCluster(x) {
    ...
    g.appendChild(createUse('#leafRedSym', { x: 60 + Number(jitter()), y: 388, class: 'sway' }));
  ...
  function layoutPlants() {
    ...
    const clusterW = 140; // wider spacing to avoid crowding
    ...
    for (let x = start; x <= end; x += clusterW) {
      ...
      const overRock = rocks.some(r => Math.abs(center - r.x) < r.r * 0.55);
      if (overRock) continue; // leave space so rocks are visible
      addCluster(x);
    }
  }
