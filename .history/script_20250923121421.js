...
(function () {
  ...
  let hookState = { phase: 'drop', targetY: 0, lastChange: 0, isRetrieving: false };
  ...
  function animateHook() {
    ...
    if (now - hookState.lastChange > 600 + Math.random() * 600) {
      ...
      hookState.isRetrieving = true; // mark this as an upward hop
      hookState.lastChange = now;
    }
    ...
    // When an upward retrieve reaches its peak, immediately drop to bottom again
    if (hookState.isRetrieving && Math.abs(clampedTop - hookState.targetY) < 3) {
      hookState.isRetrieving = false;
