...
(function () {
  ...
  function layoutPlants() {
    ...
    // Wider spacing and rock-aware gaps; compute with margins so edges are never empty
    const viewWidth = 1200; // SVG viewBox width
    const clusterW = 120;   // base spacing
    const edgeMargin = 140; // ensure coverage beyond edges to avoid clipping
    const start = -edgeMargin;
    const end = viewWidth + edgeMargin;
    ...
    for (let x = start; x <= end; x += clusterW) {
      ...
      // Keep edges filled regardless of rocks to avoid empty left/right water
      const nearEdge = (x < 60) || (x > viewWidth - 180);
      if (overRock && !nearEdge) continue; // leave more space so rocks are visible
      addCluster(x);
    }
  }
