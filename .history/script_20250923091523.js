...
(function () {
  ...
  function addCluster(x) {
    ...
    g.appendChild(createUse('#plantBlade', { x: 0, y: 398, class: 'sway' }));
    g.appendChild(createUse('#leafBroad', { x: 24, y: 390, class: 'sway fast' }));
    g.appendChild(createUse('#leafRedSym', { x: 58, y: 390, class: 'sway' }));
    g.appendChild(createUse('#plantBlade', { x: 86, y: 404, class: 'sway slow' }));
  ...
  function layoutPlants() {
    ...
    // Densify tiling and extend past edges to avoid any clipping
    const viewWidth = 1200; // SVG viewBox width
    const clusterW = 100;   // tighter spacing for fuller coverage
    const start = -2 * clusterW; // extend off-left
    const end = viewWidth + 2 * clusterW; // extend off-right
    for (let x = start; x <= end; x += clusterW) addCluster(x);
  }
