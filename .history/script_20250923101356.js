...
(function () {
  ...
  function layoutPlants() {
    ...
    // Use window width for robust early measurements
    const viewWidth = 1200;
    const pixelsPerViewUnit = Math.max(0.001, window.innerWidth / viewWidth);
    // Consistent visual spacing of ~120px
    const visualSpacingPx = 120;
    ...
    const bladeW = Math.max(28, Math.round(bladeSpacingPx / pixelsPerViewUnit));
    const bladeStart = -bladeW * 2; // extra overhang on left
    const bladeCount = Math.ceil((viewWidth + bladeW * 4) / bladeW); // overhang both sides
    if (grassBackGroup) {
      for (let i = 0; i < bladeCount; i++) {
        const x = bladeStart + i * bladeW;
        const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        g.setAttribute('transform', `translate(${x},0)`);
    ...
    const start = -clusterW * 2;
    const count = Math.ceil((viewWidth + clusterW * 4) / clusterW);
    for (let i = 0; i < count; i++) {
      const x = start + i * clusterW;
      addCluster(x);
    }
  }
