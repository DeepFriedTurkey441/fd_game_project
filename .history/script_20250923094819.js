...
(function () {
  ...
  function layoutPlants() {
    ...
    // Map rendered width to the SVG viewBox scale (1200 → bbox.width)
    const viewWidth = 1200;
    const pixelsPerViewUnit = bbox.width / viewWidth;
    ...
    const bladeStart = -bladeW;
    const bladeCount = Math.ceil((viewWidth + bladeW * 2) / bladeW);
    if (grassBackGroup) {
    ...
    const start = -clusterW;
    const count = Math.ceil((viewWidth + clusterW * 2) / clusterW);
