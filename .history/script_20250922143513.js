...
(function () {
  ...
  const rodGroup = boat ? boat.querySelector('#rodGroup') : null;
  if (!fish) return;
  ...
  let castPhase = 'idle'; // idle | windup | forward | settle
  let castStartAt = 0;
  function animateHook() {
    ...
    if (hookState.phase === 'drop') {
      ...
      // Trigger a one-time cast animation
      if (!hasCastOnce) {
        startCast(now);
      }
      return;
  ...
  function getBoatSectionIndex() {
    ...
    const w = window.innerWidth;
    const idx = Math.min(sections - 1, Math.max(0, Math.floor((centerX / w) * sections)));
    return idx; // 0..9
  }

  function startCast(now) {
    // Prevent casting when too close to right edge (sections 8 or 9)
  ...
  function stepCast(now) {
    ...
    if (castPhase === 'windup') {
      ...
      const angle = -40 * easeOutCubic(k);
      rodGroup.style.transform = `rotate(${angle}deg)`;
      if (k >= 1) { castPhase = 'forward'; castStartAt = now; }
    } else if (castPhase === 'forward') {
      // Snap rod forward; fling hook 2 sections ahead
      const dur = 260;
      const k = Math.min(1, t / dur);
      const angle = (-40) + 55 * easeOutCubic(k);
      rodGroup.style.transform = `rotate(${angle}deg)`;
      // Move hook ahead: pick target X two sections forward
      const section = getBoatSectionIndex();
      const w = window.innerWidth;
      const sectionW = w / 10;
      ...
      const nx = (curr.left + (targetX - curr.left) * easeOutCubic(k)) - boatRect.left - HOOK_EYE_X;
      hookEl.style.left = `${nx}px`;
      if (k >= 1) { castPhase = 'settle'; castStartAt = now; }
    } else if (castPhase === 'settle') {
      const dur = 300;
      const k = Math.min(1, t / dur);
      const angle = 15 - 15 * easeOutCubic(k);
      rodGroup.style.transform = `rotate(${angle}deg)`;
