...
(function () {
  ...
  function layoutPlants() {
    ...
    // approximate cluster width ~120px; add one off-screen left and right to avoid clipping
    const viewWidth = 1200; // SVG viewBox width
    const clusterW = 120;
    const start = -clusterW; // begin slightly off-left
    for (let x = start; x <= viewWidth; x += clusterW) addCluster(x);
  }
