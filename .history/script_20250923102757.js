...
(function () {
  ...
  function layoutPlants() {
    ...
    if (grassBackGroup) {
      while (grassBackGroup.firstChild) grassBackGroup.removeChild(grassBackGroup.firstChild);
    }
    // Compute rendered width of the SVG to determine cluster count
    const svg = document.getElementById('aquarium-bg');
    ...
    // Background filler thin blades for zero gaps
    const bladeSpacingPx = 44; // visual spacing in pixels
    const bladeW = Math.max(24, Math.round(bladeSpacingPx / pixelsPerViewUnit));
    ...
    if (grassBackGroup) {
      for (let i = 0; i < bladeCount; i++) {
        ...
        grassBackGroup.appendChild(g);
      }
    }
    // Start one cluster before left edge, render enough clusters to cover right edge plus one extra
    const overhang = 3; // clusters beyond each edge
    ...
    const chestZone = { x1: 1070, x2: 1170 }; // smaller reserved area around chest further right
    for (let i = 0; i < count; i++) {
      const cx = start + i * clusterW;
      const nearRock = rocks.some(r => Math.abs(cx - r.x) < r.r * 0.55);
    ...
    // Guarantee a right-edge cluster even if filters above skipped it
    const desiredEdgeX = viewWidth - Math.round(clusterW * 0.9);
    let edgeX = desiredEdgeX;
    let guardTries = 0;
    const collides = (x) => rocks.some(r => Math.abs(x - r.x) < r.r * 0.55) || (x > chestZone.x1 && x < chestZone.x2);
    while (collides(edgeX) && guardTries < 6) {
      edgeX -= Math.round(clusterW * 0.3);
      guardTries++;
    }
    addCluster(edgeX);
  }
  ...
  // Re-layout when the SVG actually changes size (accounts for zoom/devicePixelRatio changes)
  try {
    const svg = document.getElementById('aquarium-bg');
    if (window.ResizeObserver && svg) {
      const ro = new ResizeObserver(() => layoutPlants());
      ro.observe(svg);
    }
  } catch (_) {}
