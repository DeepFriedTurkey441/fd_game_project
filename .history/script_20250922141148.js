...
(function () {
  ...
  function animateHook() {
    ...
    if (hookState.phase === 'drop') {
      ...
      lineEl.style.height = (bottom - (boat.getBoundingClientRect().top + 18)) + 'px';
      hookEl.style.bottom = '-10px';
      hookState.phase = 'jig';
    ...
    const relBottom = Math.max(-10, (lineRect.bottom - clampedTop) - 7);
    hookEl.style.bottom = relBottom + 'px';

    // Reel-in effect: shorten visible line to the hook tip
    const newLineHeight = Math.max(0, (clampedTop - (lineRect.top)));
